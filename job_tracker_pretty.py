...
def add_status_dialog():
    ...
    ttk.Label(frm, text="Choose a status to append to the history (won’t change current status):").grid(row=0, column=0, columnspan=2, sticky="w", pady=(0,8))
...
def copy_company_title():
    sel = table.selection()
    if not sel:
        messagebox.showinfo("Copy", "Select a row first.")
        return
    company = table.set(sel[0], "Company")
    job = table.set(sel[0], "Job Title")
    text = f"{company} - {job}"
    root.clipboard_clear()
    root.clipboard_append(text)
    toast("Copied Company + Job Title 📋")
...
# Labels
ttk.Label(card, text="Company *").grid(row=0, column=0, sticky="w", padx=(0, PAD_BETWEEN), pady=PAD_TOPLBL)
ttk.Label(card, text="Job Title *").grid(row=0, column=1, sticky="w", padx=(0, PAD_BETWEEN), pady=PAD_TOPLBL)
ttk.Label(card, text="Date (YYYY-MM-DD)").grid(row=0, column=2, sticky="w", padx=(0, PAD_BETWEEN), pady=PAD_TOPLBL)
...
ttk.Entry(card, textvariable=link_var).grid(row=1, column=3, sticky="ew", padx=(0, PAD_BETWEEN), pady=PAD_INPUT)
ttk.Combobox(card, textvariable=status_var, values=STATUSES, state="readonly").grid(
    row=1, column=4, sticky="ew", padx=(0, 0), pady=PAD_INPUT
)
...
ttk.Button(btns, text="Add Status…", style="TButton", command=add_status_dialog).grid(row=0, column=5, padx=(0,8))
ttk.Button(btns, text="View History", style="TButton", command=view_history).grid(row=0, column=6, padx=(0,8))
ttk.Button(btns, text="Copy Company + Title", style="TButton", command=copy_company_title).grid(row=0, column=7, padx=(0,8))
...
widths = (46, 210, 240, 120, 250, 110, 96) 
...
# NEW shortcut
root.bind("<Control-Shift-C>", lambda e: copy_company_title())
